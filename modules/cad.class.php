...
class cad {
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://www.cad-comic.com/cad/');
		...
		/** Grap comics image */
                if(preg_match('/\<img src=\"(http:\/\/l3.cdn\.cad\-comic\.com\/comics\/[^\.]+.png)\" alt="([^\"]*)"[^\/]*\/\>/i', $html, $item)) {
			unset($html);
