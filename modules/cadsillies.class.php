...
/**
 * Class for downloading Ctrl + Alt + Del
...
class cadsillies {
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://www.cad-comic.com/sillies/');
		...
		/** Grap comics image */
                if(preg_match('/\<img src=\"(http:\/\/l3.cdn\.cad\-comic\.com\/comics\/[^\.]+.gif)\" alt="([^\"]*)"[^\/]*\/\>/i', $html, $item)) {
			unset($html);
			...
			if($img !== false) {
				...
				$this->title = 'Ctr+Alt+Del - Sillies: "'.$item[2].'"';
