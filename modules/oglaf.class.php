...
class oglaf {
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://oglaf.com/');
		...
		/** Grap comics image */
                if(preg_match('/\<img id\=\"strip\" src\=\"(\/media\/comic\/[^\.]+.jpg)" alt=\"([^\"]+)\" title=\"([^\"]+)\" \/\>/i', $html, $item)) {
			unset($html);
			
			$imgurl = 'http://oglaf.com/'.$item[1];
