...
/**
 * Class for downloading AmazingSuperPowers comics
...
class amazingsuperpowers {
	public $idref = 'amazingsuperpowers';
	public $title = null;
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://www.amazingsuperpowers.com/');
		...
		/** Grap comics image */
                if(preg_match('/\<img src=\"(http:\/\/www.amazingsuperpowers.com\/comics\/[^\.]+.png)\" alt=\"([^\"]*)\" title=\"([^\"]*)\" \/\>/i', $html, $item)) {
			unset($html);
			...
			if($img !== false) {
				...
				$this->title = 'AmazingSuperPowers: "'.$item[2].'"';
