...
/**
 * Class for downloading Dilbert comics
 *
...
class dilbertcom {
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://dilbert.com/strips/comic/'.date('Y-m-d'));
		...
		/** Grap comics image */
		if(preg_match('/href="(.*\.strip\.zoom\.gif)"/i', $html, $item)) {
			unset($html);

			$c = curl_init();
			curl_setopt($c, CURLOPT_URL, 'http://dilbert.com'.$item[1]);
			curl_setopt($c, CURLOPT_HEADER, false);
			curl_setopt($c, CURLOPT_USERAGENT, 'KindleGenerator/1.0');
			curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($c, CURLOPT_TIMEOUT, 15);
