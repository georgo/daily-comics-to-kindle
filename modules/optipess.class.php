...
/**
 * Class for downloading Optipess comics
...
class optipess {
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://www.optipess.com/');
		...
		/** Grap comics image */
                if(preg_match('/\<img src=\"(http:\/\/www.optipess.com\/comics\/[^\.]+.png)\" alt=\"([^\"]*)\" title=\"([^\"]*)\" \/\>/i', $html, $item)) {
			unset($html);
