...
/**
 * Class for downloading Calamities of Nature
...
class calamitiesofnature {
	public $idref = 'calamitiesofnature';
	public $title = null;
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://www.calamitiesofnature.com/');
		...
		/** Grap comics image */
                if(preg_match('/<!-- OnlineComics.net [\d]* description \"([^\"]*)\" -->.*\<IMG SRC\=\"(archive\/[^\.]+\.jpg)\" border\=\"0\" alt\=\"([^\"]*)\" title\=\"([^\"]*)\"\>/is', $html, $item)) {
			unset($html);
			
			$imgurl = 'http://www.calamitiesofnature.com/'.$item[2];
			...
			if($img !== false) {
				...
				$this->title = 'Calamities of Nature: "'.$item[1].'"';
