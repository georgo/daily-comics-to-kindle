...
/**
 * Class for downloading Red Meat comics from redmeat.com
 *
 ...
 *
 * @author Radovan Paska <carnero@carnero.cc>
...
class garfield {
	...
	public function generate($dir) {
		...
		$c = curl_init();
		curl_setopt($c, CURLOPT_URL, 'http://www.redmeat.com/redmeat/current/index.html');
		...
		if(preg_match('/<div class="stripDate">([^<]+)</div> /i', $html, $change)) {
			...
			if(file_exists('last/'.$this->idref) && file_get_contents('last/'.$this->idref) == $change[1]) {
				...
				file_put_contents('last/'.$this->idref, $change[1]);
				$c = curl_init();
				curl_setopt($c, CURLOPT_URL, 'http://www.redmeat.com/redmeat/current/index-1.gif');
				curl_setopt($c, CURLOPT_HEADER, false);
				curl_setopt($c, CURLOPT_USERAGENT, 'KindleGenerator/1.0');
				curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($c, CURLOPT_TIMEOUT, 15);
